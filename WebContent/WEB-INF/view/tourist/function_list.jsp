				...
				<tr>
					<td colspan="9"><div class="pagelist">
							<c:if test="${currpage > 1}">
								<a href="<%=path %>/courseCtrl/queryCourseByType?currpage=${currpage-1}&&courseType=1">上一页</a>
							</c:if>
							<c:if test="${currpage < pagecount}">|当前${currpage}页/总${pagecount}页|
        <a href="<%=path %>/courseCtrl/queryCourseByType?currpage=${currpage+1}&&courseType=1">下一页</a>
							</c:if>
						</div></td>
				</tr>
			</table>
