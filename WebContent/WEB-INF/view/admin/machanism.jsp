	...
	<div class="page-container">
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;"
				onclick="member_add('添加课程','<%=path%>/common/admin/machanism_add','','510')"
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加设备</a> </span> <span class="r">共有数据：<strong id="totalcount">${totalcount }</strong> 条
			</span>
		</div>
		<div class="mt-20">
			...
			<thead>
				...
				<tbody>
					<c:forEach items="${list }" var="m">
						<tr class="text-c">
							<td>${m.machanism_id }</td>
							<td>${m.machanism_name }</td>
							<td>${m.machanism_money }</td>
							<td>${m.machanism_num }</td>
							<td>${m.user_id }</td>
							<td class="td-manage"><a title="修改" href="javascript:;"
								onclick="member_edit('修改','<%=path %>/machanismCtrl/queryMachanismById?machanismId=${m.machanism_id }','','510')"
								class="ml-5" style="text-decoration: none"><i
									class="Hui-iconfont">&#xe6df;</i></a> 
								<a title="删除" href="javascript:;" onclick="member_del(this,${m.machanism_id})"
								class="ml-5" style="text-decoration: none"><i
				...
				<tr>
					<td colspan="9"><div class="pagelist">
							<c:if test="${currpage > 1}">
								<a href="<%=path %>/machanismCtrl/queryMachanism?currpage=${currpage-1}">上一页</a>
							</c:if>
							<c:if test="${currpage < pagecount}">|当前${currpage}页/总${pagecount}页|
        <a href="<%=path %>/machanismCtrl/queryMachanism?currpage=${currpage+1}">下一页</a>
							</c:if>
						</div></td>
				</tr>
			</table>
	...
	<script type="text/javascript">
		...
		function member_del(obj, id) {
			layer.confirm('确认要删除吗？', function(index) {
				$.ajax({
					type : 'POST',
					url : '<%=path %>/machanismCtrl/delMachanismById',
					data: {"machanismId":id},
					dataType : 'json',
					success : function(data) {
						if(data){
