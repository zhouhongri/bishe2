				...
				<tbody>
					<c:forEach items="${list }" var="c">
						<tr class="text-c">
							...
							<td class="td-manage"><a title="办卡" href="javascript:;"
								onclick="member_edit('办卡','<%=path %>/userCtrl/queryUserIdentityById?userId=${c.user_id }','','510')"
				...
				<tr>
					<td colspan="9"><div class="pagelist">
							<c:if test="${currpage > 1}">
								<a href="<%=path %>/userCtrl/queryTouristByIdentity?currpage=${currpage-1}">上一页</a>
							</c:if>
							<c:if test="${currpage < pagecount}">|当前${currpage}页/总${pagecount}页|
        <a href="<%=path %>/userCtrl/queryTouristByIdentity?currpage=${currpage+1}">下一页</a>
							</c:if>
						</div></td>
				</tr>
			</table>
