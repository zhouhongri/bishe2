...
<body>
	...
	<span class="c-gray en">&gt;</span> 课表中心 <span class="c-gray en">&gt;</span>
	课表管理 <a id="refresh" class="btn btn-success radius r"
				...
				<tbody>
					<c:forEach items="${list }" var="c">
						<tr class="text-c">
							<td>${c.course_id }</td>
							<td>${c.course_name }</td>
							<c:if test="${c.course_type=='0' }">
								<td>康复课程</td>
							</c:if>
							<c:if test="${c.course_type=='1' }">
								<td>功能课程</td>
							</c:if>
							<c:if test="${c.course_type=='2' }">
								<td>体形课程</td>
							</c:if>
							<td>${c.course_num }</td>
				...
				<tr>
					<td colspan="9"><div class="pagelist">
							<c:if test="${currpage > 1}">
								<a href="<%=path %>/givelessonsCtrl/queryGivelessons?currpage=${currpage-1}">上一页</a>
							</c:if>
							<c:if test="${currpage < pagecount}">|当前${currpage}页/总${pagecount}页|
        <a href="<%=path %>/givelessonsCtrl/queryGivelessons?currpage=${currpage+1}">下一页</a>
							</c:if>
						</div></td>
				</tr>
			</table>
